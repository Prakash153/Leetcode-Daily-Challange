...
Output: 68
Explanation:
This is the same example as the first but k = 3. We can select engineer 1, engineer 2 and engineer 5 to get
the maximum performance of the team. That is, performance = (2 + 10 + 5) * min(5, 4, 7) = 68.
Example 3:
  ...
  code: 
	...
	class Solution {
		private class Engineer{
			private int eff;
			private int speed;
			public Engineer(int eff, int speed)
			{
				this.eff = eff;
				this.speed = speed;
			}
		...
		public int maxPerformance(int n, int[] speed, int[] efficiency, int k) {
			...
			for(int i=0;i<n;i++)    eng.add(new Engineer(efficiency[i],speed[i]));
			eng.sort((a,b) -> b.eff - a.eff);
			PriorityQueue<Engineer> minSpeed = new PriorityQueue<>((a,b) -> a.speed- b.speed);
			long maxPerf=0, sum=0;
			for(Engineer newE : eng)
			{
				if(minSpeed.size() == k)
				{
					Engineer E = minSpeed.poll();
					sum -= E.speed;
				}
				minSpeed.add(newE);
				sum += newE.speed;
				maxPerf = Math.max(maxPerf, (sum*(long)newE.eff));
			}
